...
interface Product {
    ...
    category?: string;
    quantity?: number;
    inventoryStatus?: string | { label: string; value: string };
    rating?: number;
}
...
function saveProduct(): void {
    ...
    if (product?.value.name?.trim()) {
        if (product.value.id) {
            product.value.inventoryStatus = typeof product.value.inventoryStatus === 'object' ? product.value.inventoryStatus.value : product.value.inventoryStatus;
            ...
            const id = createId();
            product.value.id = id;
            product.value.code = id;
            product.value.image = 'product-placeholder.svg';
            product.value.inventoryStatus = typeof product.value.inventoryStatus === 'object' ? product.value.inventoryStatus.value : 'INSTOCK';
...
function deleteSelectedProducts(): void {
    if (selectedProducts.value) {
        ...
        products.value = products.value.filter((val) => !selectedIds.includes(val.id));
